...
var run = (function(dbClient) {
  function PeerServer(options) {
    ...
    if (Object.keys(this._options.ssl).length === 1) {
      util.prettyError('Warning: PeerServer will not run on an HTTPS server' + ' because either the key or the certificate has not been provided.');
    }

    this._options.ssl['name'] = 'PeerServer';
    this._app = restify.createServer(this._options.ssl);

    function definePersistedProperty(obj, key, val) {
      var closure = {key: val || {}};
      Object.defineProperty(this, key, {
        enumerable: true,
        get: function() {
          dbClient.get(key, function(err, data) {
            closure[key] = data
          });
          dbClient.set(key, closure[key]);
          return closure[key];
        },
        set: function(newVal) {
          dbClient.set(key, newVal);
          closure[key] = newVal;
        }
      });
    }
  ...
  PeerServer.prototype._initializeHTTP = function() {
    ...
    this._app.use(restify.bodyParser({
      mapParams: false
    }));
