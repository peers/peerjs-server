...
app._initializeWSS = function(server) {
  ...
  this._io.on('connection', function(socket) {
    self._log("Connected, defining conf message handler");
    ...
    socket.on('conf', function(data) {
      try {
        ...
        if (message.key != null && message.token != null && message.id != null) {
          var key = message.key;
            ...
            self._checkKey(key, ip, function(err) {
              if (!err) {
                if (!self._clients[key][id]) {
                  ...
                  self._ips[ip]++;
                  self._log("socket type: "+typeof socket);
          ...
          socket.emit('data', JSON.stringify({ type: 'ERROR', payload: { msg: 'No id, token, or key supplied to websocket server' } }));
          socket.disconnect();
          return;
        }
      } catch(e) {
