...
function PeerServer(options) {
  ...
  // Connected clients
  this._clients = storage.getItem('clients') || {};

  // Messages waiting for another peer.
  this._outstanding = storage.getItem('outstanding') || {};
  ...
  this._ips = storage.getItem('ips') || {};

  this._setCleanupIntervals();

  this._setPersistIntervals();
};
...
PeerServer.prototype._setPersistIntervals = function() {
  var self = this;

  // Persist data; default to every 10 seconds
  setInterval(function() {
    storage.setItem('clients', self._clients);
    storage.setItem('outstanding', self._outstanding);
    storage.setItem('ips', self._ips);
  }, self._options['persist_interval']);
}
