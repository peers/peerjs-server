...
app._initializeHTTP = function() {
  ...
  this.get('/:key/onlineusers', function(req, res, next) {
    res.contentType = 'application/json';
    res.send(self._getOnlineUsers(req.params.key));
  });
...
app._getOnlineUsers = function(key) {
  if (!this._clients[key]) {
    return { 'msg' : key + ' : is not a logged in user','users' : []};
  }
  
  return Object.keys(this._clients[key]);
};
